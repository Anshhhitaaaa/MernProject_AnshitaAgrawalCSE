...
export default function Header({ onLogout, onCoursesClick, onHomeClick, onCalendarClick, onTestClick, onPortfolioClick, onDashboardClick, onNotificationsClick, onProfileClick }) {
        ...
        {isMenuOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 bg-white/10 rounded-lg mt-2">
              {onHomeClick ? (
                <button type="button" onClick={() => { setIsMenuOpen(false); onHomeClick(); }} className="block w-full text-left px-3 py-2 text-white hover:text-indigo-200 transition-colors duration-200 font-medium bg-transparent border-none outline-none cursor-pointer">Home</button>
              ) : (
              ...
              <button type="button" onClick={() => { setIsMenuOpen(false); onCoursesClick(); }} className="block w-full text-left px-3 py-2 text-white hover:text-indigo-200 transition-colors duration-200 font-medium bg-transparent border-none outline-none cursor-pointer">Courses</button>
              {onCalendarClick ? (
